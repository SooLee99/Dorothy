/**
 * PR-2-S1 — Tasks 신호(read-only). ★단일 소스 hermes SQLite(~/.hermes/kanban.db) 재사용.
...
import { loadTasks } from '../kanban-store';

function readTasks(): KanbanTaskLike[] {
  return loadTasks() as unknown as KanbanTaskLike[];
}
...
export function registerTasksRoutes(app_: RouteApp, _ctx: RouteContext): void {
  ...
  app_.get('/api/tasks', (req, sendJson) => {
    ...
    sendJson(envelope({ tasks }, { sources: ['hermes/kanban.db'] }));
  });
  ...
  app_.get(/^\/api\/tasks\/([^/]+)$/, (req, sendJson) => {
    ...
    if (!t) {
      sendJson(envelope(null, { sources: ['hermes/kanban.db'], partial: true }), 404);
      return;
    }
    sendJson(envelope(toTaskDetail(t), { sources: ['hermes/kanban.db'] }));
  });
}
