/**
 * PR-2-S1 — Next 프록시: GET /api/dorothy/tasks/{id} → electron /api/tasks/{id}.
 * 작업 상세 신호(design/evidence/artifacts/session). read-only.
 */
...
export async function GET(
  ...
  const target = `http://127.0.0.1:${API_PORT}/api/tasks/${encodeURIComponent(id)}`;
  try {
