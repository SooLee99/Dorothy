/**
 * PR-2-S2 — Next 프록시: GET /api/dorothy/projects → electron /api/projects.
 * capsule + FE/BE 실프로브 + git 실측(read-only). 프로브 때문에 타임아웃 여유.
 */
...
function unavailable(reason: string) {
  return NextResponse.json({
    ...
    data: { projects: [], unavailable: true },
  });
}
...
export async function GET(req: Request) {
  ...
  const target = `http://127.0.0.1:${API_PORT}/api/projects?${url.searchParams.toString()}`;
  try {
